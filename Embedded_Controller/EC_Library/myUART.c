...
void USART_init (USART_TypeDef * USARTx, int baud_rate, int mode){ 
	...
	// Set Baudrate to 9600 using APB frequency (42MHz)
	...
	uint32_t APB_FREQ = 0;
	if(USARTx == USART1) APB_FREQ = 84000000;
	else if(USARTx == USART2) APB_FREQ = 42000000;
	else if(USARTx == USART6) APB_FREQ = 84000000;
	
	float USARTDIV = (float)APB_FREQ/16/baud_rate;
	...
	if (mode == INT) USARTx->CR1 |= USART_CR1_RXNEIE;  // Received Data Ready to be Read Interrupt
...
uint8_t USART_read (USART_TypeDef * USARTx) {
	...
	while ((USARTx->SR & USART_SR_RXNE) != USART_SR_RXNE);  // Wait until RXNE (RX not empty) bit is set
